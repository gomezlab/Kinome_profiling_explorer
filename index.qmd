...
```{r data_prep}
data_path <- here("results/combined_dose_data_summarized_no_okl_LFQ_only_annotated_v2.fst")
...
dose_1uM_summary <- all_1uM %>%
  ...
  summarize(
    median_inhibition = median(percent_inhibition, na.rm = TRUE) * 100,
...
# Join Kd Data
kd_path <- here("results/combined_kd_data_summarized_no_okl.fst")
if(file.exists(kd_path)) {
  kd_tbl <- read_fst(kd_path) %>%
    mutate(target_entrez_id = as.character(target_entrez_id)) %>%
    group_by(compound_pubchem_name, target) %>%
    summarize(kd = mean(mean_kd_apparent, na.rm = TRUE), .groups = "drop")
...
comp_stats <- agg_tbl %>%
  ...
  summarize(
    ...
    max_phase = first(max_phase),
    hits_80 = sum(median_inhibition > 80),
    .groups = "drop"
  )
...
viewer_tags <- tagList(
  ...
  (function(){
    ...
    function filteredSet(){
      ...
      return rows.sort((a,b) => b.median_inhibition - a.median_inhibition);
    }
    ...
    function renderPlot(rows){
      ...
      const x = top.map(r => v === 'kd' ? r.kd : r.median_inhibition);
